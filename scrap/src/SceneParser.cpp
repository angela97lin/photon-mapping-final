...
SceneParser::SceneParser(const std::string &filename) : _file(NULL),
                                                        _camera(NULL),
...
// Parse OBJ file and materials as a Group of objects.
Group *
SceneParser::parseOBJ()
{
    // Initialize Loader
    objl::Loader Loader;
    ...
    char token[MAX_PARSER_TOKEN_LENGTH];
    char filename[MAX_PARSER_TOKEN_LENGTH];
    char material[MAX_PARSER_TOKEN_LENGTH];
    Group *answer = new Group();
    ...
    if (loadout)
    {
        printf("properly loaded obj file: %s\n", filename);
    }
    else
    {
    ...
    for (int i = 0; i < Loader.LoadedMeshes.size(); ++i)
    {
        ...
        Vector3f diffuseOBJ = curMesh.MeshMaterial.Kd;
        Vector3f diffuse = Vector3f(diffuseOBJ.x(), diffuseOBJ.y(), diffuseOBJ.z());
        Vector3f specular = curMesh.MeshMaterial.Ks;
        float shine = curMesh.MeshMaterial.Ns;
        Material m = Material(diffuse, specular);
        for (int j = 0; j < curMesh.Indices.size(); j += 3)
        {

            // Set up triangles
            Triangle triangle(curMesh.Vertices[curMesh.Indices[j]].Position,
                              curMesh.Vertices[curMesh.Indices[j + 1]].Position,
                              curMesh.Vertices[curMesh.Indices[j + 2]].Position,
                              curMesh.Vertices[curMesh.Indices[j]].Normal,
                              curMesh.Vertices[curMesh.Indices[j + 1]].Normal,
