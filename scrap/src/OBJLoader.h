...
// Source: https://github.com/Bly7/OBJ-Loader/blob/master/Source/OBJ_Loader.h
#pragma once
...
namespace objl
{
	...
	namespace math
	{
		...
		// Vector3f Magnitude Calculation
		...
		// Angle between 2 Vector3f Objects
		...
		bool LoadFile(std::string Path)
		{
			...
			while (std::getline(file, curline))
			{
				...
				if (algorithm::firstToken(curline) == "v")
				{
					...
					// printf("Vertices: %s, %s, %s\n", spos[0].c_str(), spos[1].c_str(), spos[2].c_str());
					Vector3f vpos = Vector3f(stof(spos[0]), stof(spos[1]), stof(spos[2]));
				...
				if (algorithm::firstToken(curline) == "vt")
				{
					...
					Vector2f vtex = Vector2f(stof(stex[0]), stof(stex[1]));
				...
				if (algorithm::firstToken(curline) == "vn")
				{
					...
					Vector3f vnor = Vector3f(stof(snor[0]), stof(snor[1]), stof(snor[2]));
		...
		bool LoadMaterials(std::string path)
		{
			...
			while (std::getline(file, curline))
			{
				...
				if (algorithm::firstToken(curline) == "Ka")
				{
					...
					tempMaterial.Ka = Vector3f(std::stof(temp[0]), std::stof(temp[1]), std::stof(temp[2]));
				}
				// Diffuse Color
				if (algorithm::firstToken(curline) == "Kd")
				{
					...
					tempMaterial.Kd = Vector3f(std::stof(temp[0]), std::stof(temp[1]), std::stof(temp[2]));
				...
				if (algorithm::firstToken(curline) == "Ks")
				{
					...
					tempMaterial.Ks = Vector3f(std::stof(temp[0]), std::stof(temp[1]), std::stof(temp[2]));
