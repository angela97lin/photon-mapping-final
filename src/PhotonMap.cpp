...
1. Create photon map
    ...
    - Emit photons:
        ...
        we will emit photons randomly, then trace the photon's path (just as we might for rays in ray tracing)
            ...
            if photon hits COMPLETELY reflective (diffuse component = 0) surface:
                - don't store photon
                - bounce if possible!

            if photon hits semi diffusive 
...
Photon *PhotonMap::generatePhoton(Light *l)
{
    ...
    if (_scene.getGroup()->intersect(r, tmin, h))
    {
        ...
        p._power = h.getMaterial()->shade(r, h, tolight, intensity);
...
void PhotonMap::tracePhoton(Photon &p, int bounces)
{
    ...
    if (_scene.getGroup()->intersect(r, tmin, h))
    {
        ...
        // whether we should store the photon
        // and with what probability we should absorb, reflect, transmit (?)
        if (h.getMaterial()->isCompletelyDiffusive())
        {
            // Update values and store:
            Vector3f hitPoint = r.pointAtParameter(h.getT());
            p.x = hitPoint.x();
        ...
        if (randNum < absorptionMagnitude) // absorb
        {
            ...
            bool justSpecular = h.getMaterial()->isCompletelySpecular();
            bool justDiffuse = h.getMaterial()->isCompletelyDiffusive();

            if (!justSpecular && !justDiffuse)
            {
                storePhoton(p);
            }
            return;
        ...
        else if (randNum < absorptionMagnitude + diffuseMagnitude)
        {
        ...
        // purely specular, find a random direction.
        else
        {
                // use BRDF of surface in order to calculate new direction.
                Vector3f k_s = h.getMaterial()->getSpecularColor();
...
Vector3f PhotonMap::findRadiance(Hit h, Vector3f hitPoint, Vector3f normal)
{
    ...
    if (radius_2 < _searchRadius * _searchRadius)
    {
